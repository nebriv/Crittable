...
PLAY_TOOLS: list[dict[str, Any]] = [
    ...
    },
    {
        ...
        "description": (
            ...
            "asks ONE role a question (\"Ben — your call?\"), "
            "`set_active_roles` MUST be `role_groups=[[that_role_id]]` "
            "— one singleton group. Two roles asked independently "
            "(\"X — do A. Y — do B.\") = two singleton groups: "
    ...
    },
    {
        ...
        "description": (
            ...
            "decision — a tactical-action verb (\"isolate\", \"kill the "
            "network\", \"pull the plug\", \"quarantine\", \"contain\", "
            "\"call the regulator\", \"disable\", \"revoke\", \"reset\") "
            "is NOT a commit to inspecting a data source; they didn't "
            "ASK and they didn't NAME a data source — they ordered an "
            "action. Acknowledge via `broadcast` (Variant C) and drive "
            "the next beat; do NOT simulate the action's telemetry as "
            "a data dump. The COMMIT trigger is for verbs like "
            "\"pulling\", \"checking\", \"opening\", \"running\" that "
            "name a *data surface* being inspected, not an *action* "
            "being executed; "
            ...
            "`broadcast`; "
            "(e) to embed verdict labels, severity calls, or pre-chewed "
            "conclusions in ``data`` or ``label`` (no \"🔴 ATO "
            "CONFIRMED\", no \"Key finding:\", no \"This confirms X\") "
            "— surface raw telemetry as it would appear in the named "
            "tool; the team grades it (Block 6 Player-primacy rule, "
            "relaxed only on the ``easy`` difficulty tier). "
