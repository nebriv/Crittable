...
_TOOL_USE_PROTOCOL = (
    ...
    "  • Using `inject_event` to dump telemetry / answer a question — "
    "renders as a quiet system pill, not your reply.\n\n"
    ...
    "yet given the active roles anything to read. Pair with "
    "`broadcast` / `address_role` / `share_data` BEFORE "
    "`set_active_roles`. The engine validates this structurally and "
    "will run a recovery call to insert the missing brief — burning a "
    "retry budget — if you yield without driving.\n\n"
...
INTERJECT_NOTE = (
    ...
    "``inject_critical_event`` (interjects are not new beats).\n"
