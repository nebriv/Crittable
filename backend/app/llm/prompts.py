...
_TOOL_USE_PROTOCOL = (
    ...
    "  (a) one of {`broadcast`, `address_role`} — the player-facing message;\n"
    "  (b) one of {`set_active_roles`, `end_session`} — the yield or terminate.\n"
    "`record_decision_rationale`, `inject_event`, `inject_critical_event`, "
    "`mark_timeline_point`, `track_role_followup`, `resolve_role_followup`, "
    "`request_artifact`, `lookup_resource`, `use_extension_tool` are NEVER a "
    "valid turn on their own — they're bookkeeping / stage direction. Emit "
    "them in the SAME response as (a) and (b) when needed. The runtime "
    "exceptions (INTERJECT MODE, strict-retry recovery) override this rule "
    "explicitly when they apply.\n\n"
    "Worked example of a correct normal turn — emit ALL of these in one "
    "response (order doesn't matter to the engine; pick whichever order "
    "reads naturally):\n"
    "  • `record_decision_rationale(rationale=\"beat 1→2: SOC pulled "
    "telemetry, advancing to scope assessment\")`\n"
    "  • `broadcast(message=\"Defender shows the service account auth'd "
    "from 5 hosts in the last 90 minutes. CISO — confirm isolation; SOC "
    "— what's our first telemetry pull?\")`\n"
    "  • `set_active_roles(role_ids=[ciso.id, soc.id])`\n"
    "Counter-example — DO NOT do this (this is the failure mode the "
    "engine recovers from but burns retry budget): emit "
    "`record_decision_rationale` + `mark_timeline_point` and stop. "
    "Players see no message; the active roles have nothing to do.\n\n"
    ...
    "with at least one `broadcast` or `address_role` block on the same "
    "turn. The pending-question rule above governs the *content* of that "
    "block — answer first, then brief. `inject_event` / "
    "`inject_critical_event` / `mark_timeline_point` are FYI / pin tools; "
    "they DO NOT satisfy this rule on their own.\n\n"
    "**Stage direction is NOT a drive.** If you have just used "
    "`inject_event`, `mark_timeline_point`, or "
    "`record_decision_rationale` on this turn, you have NOT yet given "
    "the active roles a question to answer. Pair with "
    ...
    "**Record one decision rationale per normal turn.** Call "
    "`record_decision_rationale` once per *normal* play turn with one "
    "short sentence naming the plan beat and why you yielded to these "
    "roles. Skip on strict-retry attempts (the prior rationale still "
    "stands) and on INTERJECT MODE. Not a drive, not a yield."
)
...
INTERJECT_NOTE = (
    ...
    "  * DO NOT call ``end_session``, ``inject_event``, "
    "``inject_critical_event``, or ``record_decision_rationale`` "
