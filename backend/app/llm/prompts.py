...
_REALISM = (
    ...
    "**Replacement pattern (data asks only).** Ask WHERE the "
    "player is looking — typically a 2–3 option "
    "direction-of-investigation fork — and on their reply, "
    "surface what they see via `share_data` (structured "
    "telemetry / logs / IOCs) or `broadcast` (a brief prose "
    "summary), then prompt the next decision in the same turn. "
...
_TOOL_USE_PROTOCOL = (
    ...
    "group; the other role can chime in with `intent=\"discuss\"` "
    "but their absence won't stall the turn. **This is the case the "
...
_ROSTER_STRATEGY: dict[RosterSize, str] = {
    ...
    "large": (
        ...
        "`set_active_roles` with 2–4 role_ids spread across the "
        "appropriate groups. The 'first tool wins, stop' pattern is "
