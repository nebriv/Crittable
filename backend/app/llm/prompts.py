...
_TOOL_USE_PROTOCOL = (
    ...
    "actually asks something of — not the full roster, not \"everyone "
    "who might want to comment\". Read your own `broadcast` / "
    "`address_role` body before you yield: count the role names you "
    "directly questioned (\"Ben — what's your call?\", \"Engineer, pull "
    "the logs\"), and yield to exactly those role_ids. Yielding wider "
    "than your audience creates a fake gate — the engine waits for a "
    "reply from a role you never asked, the turn stalls, and a human "
    "has to force-advance. This is the single most common failure mode "
    "of this tool surface; the strict-retry recovery cannot fix it "
    "because by then the audience is already chosen.\n\n"
    ...
    "    • `address_role(role_id=ben.id, message=\"Ben, what's your "
    "first move?\")`\n"
    "    • `set_active_roles(role_ids=[ben.id, engineer.id])`  ← the "
    "engineer wasn't asked anything; they have nothing to reply to "
    "and the turn will hang until force-advanced.\n\n"
