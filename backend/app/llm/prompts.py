...
_REALISM = (
    ...
    "alert payloads and stop — no pivot menu, no 'where do "
    "you want to look next?' fork. The player reads what you "
    ...
    "`share_data` what that source shows. **If a player "
    "declines to commit** ('I don't know where to start', "
    "'you tell me') — surface the most salient artifact "
    "unprompted via `share_data` (the loudest alert, the "
    "canonical detection, the obvious first pull) and yield "
    "without a menu. Do NOT pre-package an A/B/C fork over "
    "named pivots — that pre-chews the investigation for the "
    "team and converts a capability test into a guided tour. "
    "Tactical / doctrine / stakeholder decisions (containment "
    "fork, regulator clock, holding-statement timing, "
    "board-escalation call) still go through `pose_choice` or "
    "a named A/B fork in `broadcast` exactly as Block 6 "
    "prescribes — only data-extract asks lose the fork."
)
