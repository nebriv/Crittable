...
_TOOL_USE_PROTOCOL = (
    ...
    "with a development is the right call (deliberation, processing a "
    "shock, a deliberate pause for them to discuss). The test is: can "
    "the active roles read the chat and immediately know what to type? "
    "If yes, no need to add prose. If your last visible message is just "
    "a system FYI (``inject_event`` / ``inject_critical_event`` / "
    "``mark_timeline_point`` — these render as system notes, not "
    "directives) AND the previous AI turn's open question has been "
    "answered, add a short ``broadcast`` with the next ask before "
    "yielding — otherwise players stall.\n\n"
    "Example: after an ``inject_event`` confirming a containment "
    "action, if the natural next beat is open you'd typically add "
    "``broadcast(\"Account is contained. CISO — month-end posture: "
    "hold or partially lift Finance isolation? SOC — host count when "
    "sweep completes?\")`` before ``set_active_roles``. But if CISO "
    "and SOC are clearly mid-discussion on something you just asked, "
    "yielding silently to let them finish is fine.\n\n"
