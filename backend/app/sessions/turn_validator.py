...
_STRICT_YIELD_NOTE = (
    ...
    "call `set_active_roles` with `role_groups`: each ASK in the prior "
...
_STRICT_YIELD_USER_NUDGE = (
    ...
    "now with `role_groups` matching each ASK in the narrative — "
    "singleton group per required role, multi-role group for one-of-"
    "several asks."
)
...
_DRIVE_RECOVERY_NOTE = (
    ...
    "display name. Re-read Block 10's `presence` column before "
    "drafting — address only `joined_focused` / `joined_away` seats "
    "in the broadcast prose; a clause-start address to a `not_joined` "
    "seat re-creates the wedge that brought us into recovery. "
    "Block 4 hard boundaries still apply: do NOT "
