...
def test_export_json_returns_200_with_well_formed_body(client: TestClient) -> None:
    ...
    # Issue #117 — ``flagged_for_review`` is part of the JSON
    # contract even when the fixture dict pre-dates the field; the
    # route's ``.get("flagged_for_review", [])`` default keeps the
    # response shape stable so older recordings render cleanly.
    assert body["flagged_for_review"] == []
