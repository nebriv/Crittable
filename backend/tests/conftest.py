...
We inject a dummy ``LLM_API_KEY`` so ``Settings.require_llm_api_key()``
...
``DUMMY_LLM_API_KEY`` is the load-bearing tripwire string: the
...
DUMMY_LLM_API_KEY = "dummy-key-for-tests"

os.environ.setdefault("LLM_API_KEY", DUMMY_LLM_API_KEY)
