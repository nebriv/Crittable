...
| `WORKSTREAMS_ENABLED` | `false` | Phase A chat-declutter (Phase 3, [`docs/plans/chat-decluttering.md`](plans/chat-decluttering.md) §6.8). When `true`, exposes `declare_workstreams` to the setup-tier model and surfaces `address_role.workstream_id` validation; emits `workstream_declared` WS events. When `false` (default) the feature is invisible end-to-end — the tool is hidden from the model, the prompt copy is omitted, and any `workstream_id` value emitted by a model under a stale prompt cache is dropped to `None` server-side. The AAR pipeline is workstream-blind regardless of the flag. Phase A is backend-only — flipping the flag has no UI effect; subsequent phases ship the filter pills, colored stripe, and creator-facing surfaces. |
