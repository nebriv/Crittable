...
| `INVITE_CODES` | unset (no gate) | Soft anti-strangers gate on `POST /api/sessions`. A **JSON array** of code objects — `[{"code": "acme-7Fq2", "label": "Acme Corp", "expires": "2026-12-31"}]` — where `label` and `expires` are optional (a single bare gate is `[{"code": "hunter2"}]`). When the array is non-empty, the creator must supply a code matching one of the listed, **non-expired** entries on the wizard's invite gate before a session can be created; the wizard caches the entered code in `localStorage` so refreshes don't re-prompt. Multiple codes let you hand a distinct code to each group and **rotate/revoke per group** by editing the array (it lives in the host `.env`, so it survives container restarts). `expires` (`YYYY-MM-DD`, evaluated in UTC, inclusive of that day) makes a code stop matching afterward; omit it for a code that never expires. Player join links don't need a code — they already carry per-role HMAC tokens. This is NOT a security boundary; it's a stopgap so a public-URL deploy doesn't burn LLM tokens on every drive-by. Codes are validated **inline on `POST /api/sessions`** (403 on mismatch); there is **no `?code=` verification on `GET /api/invite/status`** — that endpoint returns only `{"required": bool}` so it can't be a brute-force oracle (security audit M7). Comparison is constant-time (every code is checked with `hmac.compare_digest`, no short-circuit) and surrounding whitespace is stripped so a copy-paste with a trailing newline still matches. Code **values are never logged** — only the operator-assigned `label`, on a successful create (`create_session_invite_ok`). A malformed array fails the boot loudly (`invite_codes_invalid`); a healthy gate emits `invite_code_gate_enabled` with `code_count` + `labels` at boot. |
...
> protection — either `INVITE_CODES` set, or `SESSION_CREATE_RATE_PER_MIN`
...
3. Set `RATE_LIMIT_ENABLED=true` and tune `RATE_LIMIT_REQ_PER_MIN`. The
   ...
   default (cap 5); tune or pair with `INVITE_CODES`.
